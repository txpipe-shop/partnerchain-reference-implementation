...
#[derive(
    Serialize, Deserialize, DecodeWithMemTracking, Default, Debug, PartialEq, Eq, Clone, TypeInfo,
)]
...
impl Decode for Transaction {
    fn decode<I: parity_scale_codec::Input>(
        ...
        let function = TransactionCall::from(0);
...
impl ExtrinsicCall for Transaction {
    type Call = TransactionCall; // type info
    fn call(&self) -> &Self::Call {
        &self.function
    }
}

impl ExtrinsicMetadata for Transaction {
    const VERSIONS: &'static [u8] = &[];
    type TransactionExtensions = ();
}

impl GetDispatchInfo for Transaction {
...
impl InherentBuilder for Transaction {
    fn new_inherent(_call: Self::Call) -> Transaction {
        Transaction::from((vec![], vec![]))
    }
}

impl SignedTransactionBuilder for Transaction {
    type Address = ();
    type Signature = ();
    type Extension = ();
    fn new_signed_transaction(
        _call: Self::Call,
        _signed: Self::Address,
        _signature: Self::Signature,
        _tx_ext: Self::Extension,
    ) -> Transaction {
        Transaction::from((vec![], vec![]))
    }
}
...
impl From<(Vec<Input>, Vec<Output>)> for Transaction {
    fn from((inputs, outputs): (Vec<Input>, Vec<Output>)) -> Self {
        Self {
            ...
            function: TransactionCall::default(),
        }
    }
}
...
impl Value {
    ...
    /// Checks if a given asset is contained in the `Value`.
    pub fn contains_asset(&self, asset_name: &AssetName, asset_policy: &PolicyId) -> bool {
        if let Value::Multiasset(_, ma) = &self {
            if let Some(assets) = ma.0.get(asset_policy) {
                return assets.0.contains_key(asset_name);
            }
        }
        false
    }
...
pub struct TransactionCall(pub u64);

impl Dispatchable for TransactionCall {
...
impl From<u64> for TransactionCall {
    fn from(value: u64) -> Self {
        Self(value)
    }
}

impl From<Transaction> for TransactionCall {
    fn from(_tx: Transaction) -> Self {
        Self(0)
    }
}
