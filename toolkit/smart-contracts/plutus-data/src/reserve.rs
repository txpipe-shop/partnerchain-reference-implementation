            ...
            datum: {
                let (policy_id_bytes, asset_name_bytes) = {
                    let AssetId {
                        policy_id,
                        asset_name,
                    } = value.immutable_settings.token.clone();
                ...
                v_function_hash_and_initial_incentive.add(&PlutusData::new_bytes(
                    value
                        .mutable_settings
                        .total_accrued_function_asset_name
                        .0
                        .to_vec(),
                ));
...
mod tests {
    ...
    fn test_reserve_datum() -> ReserveDatum {
        ReserveDatum {
            ...
            stats: ReserveStats {
                token_total_amount_transferred: 1000,
            },
        }
    }
    ...
    fn encode_reserve_datum() {
        assert_eq!(
            PlutusData::from(test_reserve_datum()),
            test_reserve_datum_plutus_data()
        )
    }
