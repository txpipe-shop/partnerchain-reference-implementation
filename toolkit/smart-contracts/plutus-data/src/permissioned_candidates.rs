...
use crate::{
    candidate_keys::*, DataDecodingError, DecodingResult, VersionedDatum, VersionedDatumWithLegacy,
...
impl From<PermissionedCandidateDatumV1> for PermissionedCandidateData {
    ...
    fn from(value: PermissionedCandidateDatumV1) -> Self {
        let PermissionedCandidateDatumV1 {
            partner_chains_key,
            keys,
        } = value;
...
pub fn permissioned_candidates_to_plutus_data(
    ...
    fn candidates_to_plutus_data_v0(candidates: &[PermissionedCandidateData]) -> PlutusData {
        ...
        for candidate in candidates {
            ...
            candidate_datum.add(&PlutusData::new_bytes(
                candidate.sidechain_public_key.0.clone(),
            ));
    ...
    if candidates
        .iter()
        .all(|c| c.keys.has_only_aura_and_grandpa_keys())
    {
...
fn decode_v1_candidate_datum(datum: &PlutusData) -> Option<PermissionedCandidateDatumV1> {
    ...
    Some(PermissionedCandidateDatumV1 {
        partner_chains_key,
        keys,
    })
}
...
mod tests {
    ...
    fn valid_legacy_permissioned_candidates() {
        ...
        assert_eq!(
            PermissionedCandidateDatums::try_from(plutus_data).unwrap(),
            expected_datum
        )
    }
    ...
    fn permissioned_candidates_to_plutus_data_outputs_v0_for_aura_and_grandpa_keys() {
        ...
        assert_eq!(
            permissioned_candidates_to_plutus_data(&domain_data),
            expected_plutus_data
        )
    }
