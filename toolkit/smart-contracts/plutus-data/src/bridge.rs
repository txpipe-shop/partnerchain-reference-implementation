...
use crate::*;
use cardano_serialization_lib::{traits::NoneOrEmpty, PlutusData};
...
impl VersionedDatum for TokenTransferDatum {
    fn decode(data: &PlutusData) -> crate::DecodingResult<Self> {
        match plutus_data_version_and_payload(data) {
            None => Err(decoding_error_and_log(
                data,
                "TokenTransferDatum",
                "unversioned datum",
            )),
            ...
            Some(_) => Err(decoding_error_and_log(
                data,
                "TokenTransferDatum",
                "invalid version",
            )),
...
fn decode_v1_token_transfer_datum(appendix: &PlutusData) -> Option<TokenTransferDatum> {
    ...
    match alternative {
        0 if data.len() == 1 => {
            ...
            Some(TokenTransferDatum::V1(TokenTransferDatumV1::UserTransfer {
                receiver,
            }))
        }
        1 if data.is_none_or_empty() => Some(TokenTransferDatum::V1(
            TokenTransferDatumV1::ReserveTransfer,
        )),
        _ => None,
    }
...
mod tests {
    ...
    mod decode {
        ...
        fn reserve_transfer_v1() {
            ...
            assert_eq!(
                datum,
                TokenTransferDatum::V1(TokenTransferDatumV1::ReserveTransfer)
            )
        }
    }
