...
use crate::{decoding_error_and_log, DataDecodingError, DecodingResult};
    ...
    fn try_from(datum: PlutusData) -> DecodingResult<Self> {
        let error = || {
            decoding_error_and_log(
                &datum,
                "GovernedMapDatum",
                "Expected List([UTF8String, Bytes])",
            )
        };
...
pub fn governed_map_datum_to_plutus_data(governed_map_datum: &GovernedMapDatum) -> PlutusData {
    ...
    list.add(&PlutusData::new_bytes(
        governed_map_datum.key.as_bytes().to_vec(),
    ));
...
mod tests {
    ...
    fn governed_map_from_plutus_data() {
        ...
        let expected_datum = GovernedMapDatum {
            key: "key".to_string(),
            value: hex!("ddeeff").to_vec().into(),
        };

        assert_eq!(
            GovernedMapDatum::try_from(plutus_data).unwrap(),
            expected_datum
        )
    }
    ...
    fn governed_map_to_plutus_data() {
        let governed_map_datum = GovernedMapDatum {
            key: "key".to_string(),
            value: hex!("ddeeff").to_vec().into(),
        };
