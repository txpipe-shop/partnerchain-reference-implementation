...
use sidechain_domain::{
    byte_string::ByteString, AdaBasedStaking, AssetId, AssetName, AuraPublicKey, CandidateKeys,
...
const GOVERNANCE_AUTHORITY: MainchainKeyHash = MainchainKeyHash(hex!(
...
const EVE_PUBLIC_KEY: StakePoolPublicKey = StakePoolPublicKey(hex!(
    "a5ab6e82531cac3480cf7ff360f38a0beeea93cabfdd1ed0495e0423f7875c57"
));

const EVE_PUBLIC_KEY_HASH: MainchainKeyHash = MainchainKeyHash(hex!(
...
const V_FUNCTION_HASH: PolicyId = PolicyId(hex!(
    "ef1eb7b85327a8460799025a5affd0a8d8015731e9aacd5d1106a82b"
));
const V_FUNCTION_UTXO: UtxoId = UtxoId {
    tx_hash: McTxHash(hex!(
        "f8fbe7316561e57de9ecd1c86ee8f8b512a314ba86499ba9a584bfa8fe2edc8d"
    )),
    index: UtxoIndex(6),
};

const REWARDS_TOKEN_POLICY_ID: PolicyId = PolicyId(hex!(
    "1fab25f376bc49a181d03a869ee8eaa3157a3a3d242a619ca7995b2b"
));
...
async fn upsert_d_param() {
    ...
    .is_none());
    assert_eq!(
        d_param::get_d_param(genesis_utxo, &client)
            .await
            .unwrap()
            .unwrap(),
        DParameter {
...
async fn upsert_permissioned_candidates() {
    ...
    let network = client
        ...
        .to_csl();
    assert!(
        run_upsert_permissioned_candidates(genesis_utxo, 77, &client)
            .await
            .is_some()
    );
    assert_eq!(
    ...
    assert!(
        run_upsert_permissioned_candidates(genesis_utxo, 231, &client)
            .await
            .is_some()
    )
}
...
async fn register() {
    ...
    assert!(run_register(genesis_utxo, signature.clone(), &client)
        .await
        .is_some());
    assert!(run_register(genesis_utxo, signature, &client)
        .await
        .is_none());
    assert!(run_register(genesis_utxo, other_signature, &client)
        .await
        .is_some());
}
...
async fn governed_map_operations() {
    ...
    assert!(
        result1.is_ok_and(|x| x.is_some()),
        "First key-value insertion should succeed"
    );
    ...
    assert!(
        result3.is_err(),
        "Inserting the same key with a different value should fail"
    );
    ...
    assert!(
        result4.is_ok_and(|x| x.is_some()),
        "Inserting a different key should succeed"
    );
    ...
    .await;
    assert!(
        result_force.is_ok_and(|x| x.is_some()),
        "force insertion succeed"
    );
    // Remove a key that exists, should succeed
    let remove_result1 = remove(key4.clone()).await;
    assert!(
        remove_result1.is_ok_and(|x| x.is_some()),
    ...
    assert_eq!(
        get_key1_result,
        Some(value1),
        "Existing key value should be returned by get"
    );

    let get_removed_key_result = run_get(genesis_utxo, key4, &client).await.unwrap();
    assert_eq!(
        get_removed_key_result, None,
        "Get for non-existent key should return None"
    );
}
...
async fn governed_map_update() {
    ...
    assert!(
        result.is_err(),
        "Updating an existing key should fail with incorrect expected value"
    );
}
...
async fn await_ogmios(ogmios_port: u16) -> Result<OgmiosClients, String> {
    ...
    Retry::spawn(
        FixedInterval::new(Duration::from_millis(100)).take(1000),
        || async {
...
>(
    ...
    let governance_utxos = client
        .query_utxos(&[GOVERNANCE_AUTHORITY_ADDRESS.to_string()])
        .await
        .unwrap();
...
async fn run_register<T: QueryLedgerState + Transactions + QueryNetwork + QueryUtxoByUtxoId>(
    ...
    let eve_utxos = client
        .query_utxos(&[EVE_ADDRESS.to_string()])
        .await
        .unwrap();
        ...
        .flat_map(|utxo| {
            utxo.value
                ...
                .and_then(|assets| {
                    assets
                        .iter()
                        .find(|a| a.name == token_asset_name.0.to_vec())
                })
...
>(
    ...
    let scripts_data = scripts_data::get_scripts_data_with_ogmios(genesis_utxo, client)
        .await
        .unwrap();
...
>(
    ...
    result
        .iter()
        .for_each(|x| x.iter().for_each(cleanup_temp_wallet_file));
    result
}
...
>(
    ...
    let result = run_insert_with_force(
        genesis_utxo,
        key,
        value,
        payment_signing_key,
