...
impl GovernancePolicyScript {
    ...
    pub(crate) fn is_single_key_policy_for(&self, key_hash: &Ed25519KeyHash) -> bool {
        match self {
            Self::MultiSig(PartnerChainsMultisigPolicy {
                script: _,
                key_hashes,
                threshold,
            }) => {
                *threshold == 1
                    ...
                    && key_hashes
                        .iter()
                        .any(|h| &Ed25519KeyHash::from(*h) == key_hash)
            }
            Self::AtLeastNNativeScript(SimpleAtLeastN {
                threshold,
                key_hashes,
            }) => {
                *threshold == 1
...
impl GovernanceData {
    ...
    async fn get_governance_utxo<T: QueryLedgerState + QueryNetwork>(
        ...
        let network = client
            ...
            .map_err(|e| {
                JsError::from_str(&format!(
                    "Could not get Shelley Genesis Configuration: {}",
                    e
                ))
            })?
            .network;
        ...
        Ok(utxos.into_iter().find(|utxo| {
            ...
            let contains_version_oracle_token = utxo
                .value
                .native_tokens
                .contains_key(&version_oracle_data.policy_id().0);
...
fn parse_pc_multisig(script: plutus_script::PlutusScript) -> Option<GovernancePolicyScript> {
    script
        ...
        .and_then(|list| {
            ...
            Some(GovernancePolicyScript::MultiSig(
                PartnerChainsMultisigPolicy {
                    script,
                    key_hashes,
                    threshold,
                },
...
fn parse_simple_at_least_n_native_script(
    ...
    match script.json {
        Some(OgmiosNativeScript::Some { from, at_least }) => {
            ...
            Some(GovernancePolicyScript::AtLeastNNativeScript(
                SimpleAtLeastN {
                    threshold: at_least,
                    key_hashes: keys,
                },
        ...
        match value {
            GovernancePolicyScript::MultiSig(PartnerChainsMultisigPolicy {
                ...
                key_hashes: key_hashes
                    .iter()
                    .map(|h| ByteString::from(h.to_vec()))
                    .collect(),
            },
...
impl MultiSigParameters {
    ...
    pub fn new(governance_authorities: &[MainchainKeyHash], threshold: u8) -> Result<Self, &str> {
        ...
        Ok(Self {
            governance_authorities: governance_authorities.to_vec(),
            threshold,
        })
    }
    ...
    pub fn new_one_of_one(governance_authority: &MainchainKeyHash) -> Self {
        Self {
            governance_authorities: vec![*governance_authority],
            threshold: 1,
        }
    }
