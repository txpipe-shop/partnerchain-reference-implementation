...
pub fn key_hash_address(pub_key_hash: &Ed25519KeyHash, network: NetworkIdKind) -> Address {
    EnterpriseAddress::new(
        network_id_kind_to_u8(network),
        &Credential::from_keyhash(pub_key_hash),
    )
    .to_address()
}
...
pub(crate) fn convert_cost_models(m: &PlutusCostModels) -> Costmdls {
    ...
    mdls.insert(
        ...
        &CostModel::from(m.plutus_v1.to_owned()),
    );
    mdls.insert(
        &Language::new_plutus_v2(),
        &CostModel::from(m.plutus_v2.to_owned()),
    );
    mdls.insert(
        &Language::new_plutus_v3(),
        &CostModel::from(m.plutus_v3.to_owned()),
    );
    mdls
}
...
pub(crate) fn convert_reference_script_costs(
    ...
    let r = Ratio::<u64>::from_f64(costs.base).ok_or_else(|| {
        JsError::from_str(&format!(
            "Failed to decode cost base {} as a u64 ratio",
            costs.base
        ))
    })?;
...
impl OgmiosUtxoExt for OgmiosUtxo {
    fn to_csl_tx_input(&self) -> TransactionInput {
        TransactionInput::new(
            &TransactionHash::from(self.transaction.id),
            self.index.into(),
        )
    }
        ...
        if let Some(script) = self.script.clone() {
            let plutus_script_ref_opt =
                script
                    ...
                    .try_into()
                    .ok()
                    .map(|plutus_script: PlutusScript| {
    ...
    fn to_csl(&self) -> Result<TransactionUnspentOutput, JsError> {
        Ok(TransactionUnspentOutput::new(
            &self.to_csl_tx_input(),
            &self.to_csl_tx_output()?,
        ))
    }
...
impl TransactionContext {
    ...
    pub(crate) async fn for_payment_key<C: QueryLedgerState + QueryNetwork>(
        ...
        let payment_key_utxos = client
            .query_utxos(&[payment_address.to_bech32(None)?])
            .await?;
    ...
    pub(crate) fn sign(&self, tx: &Transaction) -> Transaction {
        ...
        vkeywitnesses.add(&Vkeywitness::new(
            &Vkey::new(&self.payment_key.0.to_public()),
            &signature,
        ));
...
impl TransactionBuilderExt for TransactionBuilder {
    fn add_output_with_one_script_token(
        ...
        let output = amount_builder
            .with_minimum_ada_and_asset(&ma, ctx)?
            .build()?;
        ...
        fn max_possible_collaterals(ctx: &TransactionContext) -> Vec<OgmiosUtxo> {
            ...
            utxos
                ...
                .take(
                    max_inputs
                        .try_into()
                        .expect("max_collateral_input fit in usize"),
                )
                .collect()
        }
...
impl Script {
    ...
    pub(crate) fn script_hash(&self) -> [u8; 28] {
        match self {
            ...
            Self::Native(script) => script
                .hash()
                .to_bytes()
                .try_into()
                .expect("CSL script hash is always 28 bytes"),
        }
    }
}
    ...
    fn convert_value_with_multi_asset_test() {
        let ogmios_value = OgmiosValue {
            ...
            native_tokens: vec![
                ...
                (
                    ...
                    vec![
                        Asset {
                            name: hex!("222222").to_vec(),
                            amount: 222,
                        },
                        Asset {
                            name: hex!("333333").to_vec(),
                            amount: 333,
                        },
                    ],
                ),
        ...
        assert_eq!(
            multiasset.get_asset(
                &[1u8; 28].into(),
                &AssetName::new(hex!("222222").to_vec()).unwrap()
            ),
            222u64.into()
        );
        assert_eq!(
            multiasset.get_asset(
                &[1u8; 28].into(),
                &AssetName::new(hex!("333333").to_vec()).unwrap()
            ),
            333u64.into()
        );
    }
...
mod prop_tests {
    ...
    fn selected_collateral_inputs_equal_total_collateral_and_collateral_return(
        ...
        let collateral_inputs_sum: u64 = sum_lovelace(&match_inputs(
            &tx.body().collateral().unwrap(),
            &payment_utxos,
        ));
        let collateral_return: u64 = tx
            .body()
            .collateral_return()
            .unwrap()
            .amount()
