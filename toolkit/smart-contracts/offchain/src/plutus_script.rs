...
impl PlutusScript {
    ...
    pub fn asset(
        ...
        Ok(AssetId {
            policy_id: self.policy_id(),
            asset_name: AssetName::from_csl(asset_name)?,
        })
    }
...
pub(crate) mod tests {
    ...
    pub(crate) const TEST_GENESIS_UTXO: UtxoId = UtxoId {
    ...
    fn apply_parameters_to_deregister() {
        ...
        assert_eq!(
            hex::encode(applied.bytes),
            hex::encode(CANDIDATES_SCRIPT_WITH_APPLIED_PARAMS)
        );
    }
