...
use sidechain_domain::{crypto::blake2b, McTxHash};
...
impl TemporaryWallet {
    pub(crate) fn address_bech32(&self) -> String {
        self.address
            .to_bech32(None)
            .expect("to_bech32 is safe with None prefix")
    }
}
...
fn estimate_required_value(
    ...
    total_input
        .clamped_sub(&change)
        .checked_add(&Value::new(&5_000_000u32.into()))
}
...
async fn transfer_to_temporary_wallet<T: Transactions + QueryUtxoByUtxoId, A: AwaitTx>(
    ...
    client
        .submit_transaction(&payment_ctx.sign(&funding_tx).to_bytes())
        .await?;
    ...
    A: AwaitTx,
{
    Ok(
        if governance_data
            .policy
            .is_single_key_policy_for(&payment_ctx.payment_key_hash())
        {
    ...
    A: AwaitTx,
{
    ...
    await_tx
        .await_tx_output(client, McTxHash(res.transaction.id))
        .await?;
    Ok(tx_id)
}
    ...
    A: AwaitTx,
{
    ...
    let tx = Costs::calculate_costs(|c| make_tx(c, &temp_wallet_ctx), client)
        .await
        .map_err(|e| {
            anyhow::anyhow!(
    ...
    let signed_tx_by_caller = if governance_data
        .policy
        .contains_authority(&original_ctx.payment_key_hash())
    {
