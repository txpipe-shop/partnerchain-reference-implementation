...
fn version_oracle_validator_address() -> Address {
    Address::from_bech32(
        &version_oracle_validator()
            .address_bech32(tx_context().network)
            .unwrap(),
    )
    .unwrap()
}
...
fn governance_utxo() -> OgmiosUtxo {
    OgmiosUtxo {
        ...
        datum: Some(Datum {
            bytes: version_oracle_policy_hash().to_vec(),
        }),
        ..OgmiosUtxo::default()
    }
}
...
fn new_governance_authority() -> MainchainKeyHash {
    MainchainKeyHash(hex_literal::hex!(
    ...
    assert_eq!(
        ...
        PlutusData::new_list(&{
            ...
            list.add(&PlutusData::new_bytes(
                version_oracle_policy_hash().to_vec(),
            ));
            list
        })
    );
...
fn contains_correct_redeemers() {
    let redeemers = test_update_governance_tx()
        .witness_set()
        .redeemers()
        .unwrap();
    ...
    assert_eq!(
        spend_redeemer.data(),
        PlutusData::new_integer(&32u64.into())
    );
