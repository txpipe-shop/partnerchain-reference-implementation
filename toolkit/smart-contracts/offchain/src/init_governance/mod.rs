use crate::{
    await_tx::AwaitTx, cardano_keys::CardanoPaymentSigningKey, csl::key_hash_address, csl::Costs,
...
>(
    ...
    log::info!(
        "✉️ Submitter address: {}",
        own_address.to_bech32(None).unwrap()
    );
    ...
    let genesis_utxo = match genesis_utxo_id {
        None => {
            ...
            let utxo = own_utxos
                .first()
                .ok_or(anyhow!("No UTXOs to choose from"))?
                .clone();
    ...
    let result = client
        .submit_transaction(&signed_transaction.to_bytes())
        .await?;
    ...
    Ok(InitGovernanceResult {
        tx_hash: McTxHash(tx_id),
        genesis_utxo: genesis_utxo.utxo_id(),
    })
}
