...
pub(crate) fn init_governance_transaction(
    ...
    let multi_sig_policy = governance_parameters
        .as_simple_at_least_n()
...
pub(crate) fn version_oracle_datum_output(
    ...
    let ma = MultiAsset::new().with_asset_amount(
        &version_oracle_policy.asset(version_oracle_asset_name())?,
        1u64,
    )?;

    let output = amount_builder
        .with_minimum_ada_and_asset(&ma, tx_context)?
        .build()?;
    Ok(output)
}
