...
mod governed_map_insert_tx_tests {
    ...
    fn test_costs() -> Costs {
        Costs::new(
            vec![
                (
                    ScriptHash::from_bytes(token_policy_id().to_vec()).unwrap(),
                    policy_ex_units(),
                ),
    ...
    fn change_is_returned_correctly() {
        ...
        assert!(
            coins_sum <= total_input,
            "Sum of outputs plus fee should not exceed total input"
        );
    }
    ...
    fn should_send_key_value_token_to_validator_address() {
        ...
        let output_multi_asset = script_output
            .amount()
            .multiasset()
            .expect("Utxo should contain a native token");
    ...
    fn attaches_correct_plutus_data_at_validator_address() {
        ...
        let plutus_data = script_output
            .plutus_data()
            .expect("Utxo should have plutus data attached");
...
mod get_current_value_tests {
    ...
    fn returns_none_when_utxo_without_token() {
        let utxo = OgmiosUtxo {
            ...
            value: OgmiosValue {
                lovelace: 1000000,
                native_tokens: vec![].into_iter().collect(),
            },
...
mod get_utxos_for_key_tests {
    ...
    fn returns_utxo_when_matches_key_and_has_token() {
        ...
        let result = get_utxos_for_key(
            vec![expected_utxo.clone()],
            test_key(),
...
mod governed_map_remove_tx_tests {
    ...
    fn multiple_inputs_are_consumed_when_removing_multiple_utxos() {
        ...
        assert_eq!(
            payment_input.index(),
            0,
            "First input should be payment UTXO"
        );
    }
    ...
    fn transaction_has_no_output_to_validator_address() {
        ...
        let validator_outputs = outputs
            .into_iter()
            .filter(|o| o.address() == validator_addr())
            .count();
    ...
    OgmiosUtxo {
        ...
        value: OgmiosValue {
            ...
            native_tokens: vec![(
                ...
                vec![OgmiosAsset {
                    name: version_oracle_token_name(),
                    amount: 1,
                }],
            )]
            .into_iter()
        ...
        datum: Some(Datum {
            bytes: PlutusData::new_integer(&(32u64.into())).to_bytes(),
        }),
        ..Default::default()
    }
}
