...
>(
    ...
    let validator_utxos = ogmios_client
        .query_utxos(&[scripts.validator_address.clone()])
        .await?;
...
fn insert_key_value_tx(
    ...
    Ok(tx_builder
        .balance_update_and_build(ctx)?
        .remove_native_script_witnesses())
}
...
>(
    ...
    {
        return Err(anyhow!(
            "Value for key '{key}' is set to a different value than expected."
        ));
    }
...
pub async fn run_list<C: QueryLedgerState + QueryNetwork + Transactions + QueryUtxoByUtxoId>(
    ...
    let validator_utxos = ogmios_client
        ...
        .await?;
    Ok(
        ogmios_utxos_to_governed_map_utxos(validator_utxos.into_iter(), scripts.policy_id())
            .map(|(_, datum)| datum),
    )
}
...
fn ogmios_utxos_to_governed_map_utxos(
    ...
    utxos.flat_map(move |utxo| {
        ...
        GovernedMapDatum::try_from(datum_plutus_data)
            .ok()
            .and_then(|d| Some((utxo, d)))
    })
}
