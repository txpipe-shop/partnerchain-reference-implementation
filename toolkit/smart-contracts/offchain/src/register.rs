...
>(
    ...
    let own_registration_utxos = own_registrations
        .iter()
        .map(|r| r.0.clone())
        .collect::<Vec<_>>();
    ...
    log::info!(
        ...
        hex::encode(result.transaction.id)
    );
...
fn get_candidate_registration(validator_utxo: OgmiosUtxo) -> anyhow::Result<CandidateRegistration> {
    let datum = validator_utxo
        .datum
        .ok_or_else(|| anyhow!("UTXO does not have a datum"))?;
...
mod tests {
    ...
    fn output_at_validator_has_register_candidate_datum(
        ...
        let validator_output = outputs
            .into_iter()
            .find(|o| o.address() == *validator_address)
            .unwrap();
        assert_eq!(
