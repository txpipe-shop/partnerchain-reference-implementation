...
impl CardanoPaymentSigningKey {
    ...
    pub fn from_extended_128_bytes(bytes: [u8; 128]) -> anyhow::Result<Self> {
        ...
        Ok(Self(PrivateKey::from_extended_bytes(&prefix).map_err(
            |e| anyhow!("Couldn't parse 128 bytes into a BIP32 Private Key: {e}"),
        )?))
    }
    ...
    pub fn from_normal_bytes(bytes: [u8; 32]) -> Result<Self, anyhow::Error> {
        Ok(Self(PrivateKey::from_normal_bytes(&bytes).map_err(
            |e| anyhow!("Couldn't parse 32 bytes into a Private Key: {e}"),
        )?))
    }
...
impl TryFrom<CardanoKeyFileContent> for CardanoPaymentSigningKey {
    ...
    fn try_from(key: CardanoKeyFileContent) -> Result<Self, Self::Error> {
        ...
        if key_type == "PaymentSigningKeyShelley_ed25519" {
            Ok(CardanoPaymentSigningKey::from_normal_bytes(
                key.raw_key_bytes()?,
            )?)
        } else if key_type == "PaymentExtendedSigningKeyShelley_ed25519_bip32" {
            Ok(CardanoPaymentSigningKey::from_extended_128_bytes(
                key.raw_key_bytes()?,
            )?)
        } else {
            Err(anyhow!(
                "Unsupported key type: {}. Expected a signing key",
                key_type
            ))
        }
    }
