use crate::tests::{
    runtime::MockRuntime, MockIO, MockIOContext, CHAIN_CONFIG_FILE_PATH, RESOURCES_CONFIG_FILE_PATH,
};
...
fn happy_path() {
    ...
    let result = StartNodeCmd::<MockRuntime> {
        silent: false,
    ...
    verify_json!(
        ...
        expected_final_resources_config_json()
    );
}
...
mod load_chain_config {
    use crate::{
        ...
        tests::{MockIO, MockIOContext, CHAIN_CONFIG_FILE_PATH},
    };
    ...
    fn rejects_incorrect_config() {
        ...
        assert!(
            ...
            "should not return a config value"
        );
    }
}
