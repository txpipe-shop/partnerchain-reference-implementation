...
use crate::config::{config_fields, ChainConfig, ConfigFieldDefinition, ConfigFile};
...
use crate::{cardano_key, CmdRun, PartnerChainRuntime};
use anyhow::anyhow;
use anyhow::Context;
...
fn initial_permissioned_candidates_from_chain_config<C: IOContext, Keys: MaybeFromCandidateKeys>(
    ...
    let candidates = candidates
        .iter()
        .map(PermissionedCandidateData::from)
        .collect();
