...
impl Display for PermissionedCandidateKeys {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Partner Chains Key: {}",
            self.partner_chains_key.to_hex_string()
        )?;
    ...
    for ParsedPermissionedCandidatesKeys<Keys>
{
    ...
    fn try_from(value: &PermissionedCandidateKeys) -> Result<Self, Self::Error> {
        let sidechain =
            parse_ecdsa(&value.partner_chains_key.0).ok_or(anyhow::Error::msg(format!(
                "'{}' is invalid ECDSA public key",
                value.partner_chains_key.to_hex_string()
            )))?;
