...
pub(crate) fn network_key_path(substrate_node_base_path: &str) -> String {
    format!(
        "{}/secret_ed25519",
        network_key_directory(substrate_node_base_path)
    )
}
...
impl<T: PartnerChainRuntime> CmdRun for GenerateKeysCmd<T> {
    fn run<C: IOContext>(&self, context: &C) -> anyhow::Result<()> {
        ...
        context.eprint(&format!(
            "→ {} {} key",
            CROSS_CHAIN.scheme, CROSS_CHAIN.name
        ));
...
pub(crate) fn generate_network_key<C: IOContext>(
    ...
    let maybe_existing_key = context
        .read_file(&config.network_key_path())
        .as_deref()
        .map(decode_network_key);
...
fn generate_keys<C: IOContext>(
    ...
    let output = context.run_command(&format!(
        "{executable} key generate --scheme {scheme} --output-type json"
    ))?;
...
fn store_keys<C: IOContext>(
    context: &C,
    GenerateKeysConfig {
        substrate_node_base_path: base_path,
    }: &GenerateKeysConfig,
...
fn generate_or_load_key<C: IOContext>(
    ...
    let key: anyhow::Result<String> = if let Some(key) = find_existing_key(&existing_keys, key_def)
    {
        if context.prompt_yes_no(
            &format!(
                "A {} key already exists in store: {key} - overwrite it?",
                key_def.name
            ),
            false,
        ) {
            ...
            context.delete_file(&old_key_path).context(format!(
                "Failed to overwrite {} key at {old_key_path}",
                key_def.name
            ))?;
