...
pub(crate) fn get_payment_verification_key_bytes_from_file(
    path: &str,
    context: &impl IOContext,
) -> anyhow::Result<[u8; 32]> {
    let key_file = parse_json_key_file(path, context)?;
    ...
    if key_type == "PaymentExtendedVerificationKeyShelley_ed25519_bip32" {
        ...
        let prefix: [u8; 32] = key_bytes[0..32]
            .try_into()
            .expect("[0..32] of [u8; 64] has to fit [u8; 32]");
        Ok(prefix)
...
pub(crate) fn get_mc_staking_signing_key_from_file(
    ...
    if key_type == "StakePoolSigningKey_ed25519" {
        ...
        Err(anyhow!(
            "Expected Stake Pool Signing Key, but got '{key_type}' in {path}"
        ))
    }
}

pub(crate) fn get_stake_pool_verification_key_from_file(
    path: &str,
    context: &impl IOContext,
    ...
    if key_type == "StakePoolVerificationKey_ed25519" {
        ...
        Err(anyhow!(
            "Expected Stake Pool Verification Key, but got '{key_type}' in {path}"
        ))
    }
}
