...
impl MockIO {
    ...
    pub fn run_command(expected_cmd: &str, output: &str) -> Self {
        Self::RunCommand {
            expected_cmd: expected_cmd.into(),
...
impl OffchainMocks {
    pub(crate) fn new_with_mock(addr: &str, mock: OffchainMock) -> Self {
        Self {
            mocks: vec![(addr.into(), mock)].into_iter().collect(),
        }
    }
}
...
pub struct OffchainMock {
    ...
    pub upsert_permissioned_candidates: HashMap<
        (
            UtxoId,
            Vec<sidechain_domain::PermissionedCandidateData>,
            PrivateKeyBytes,
        ),
        Result<Option<MultiSigSmartContractResult>, String>,
    >,
...
impl OffchainMock {
    ...
    pub(crate) fn with_scripts_data(
        ...
        Self {
            scripts_data: vec![(genesis_utxo, scripts_data)].into_iter().collect(),
            ..self
        }
    }
    ...
    pub(crate) fn with_get_d_param(
        ...
        Self {
            get_d_param: [(genesis_utxo, result)].into(),
            ..self
        }
    }
    ...
    pub(crate) fn with_upsert_d_param(
        ...
        Self {
            upsert_d_param: [((genesis_utxo, d_param, payment_key), result)].into(),
            ..self
        }
    }
    ...
    pub(crate) fn with_deregister(
        ...
        Self {
            deregister: [(
                (genesis_utxo, payment_signing_key, stake_ownership_pub_key),
                result,
            )]
            .into(),
            ..self
    ...
    pub(crate) fn with_get_permissioned_candidates(
        ...
        Self {
            get_permissioned_candidates: [(genesis_utxo, result)].into(),
            ..self
        }
    }
...
impl GetPermissionedCandidates for OffchainMock {
    async fn get_permissioned_candidates(
        ...
        self.get_permissioned_candidates
            ...
            .unwrap_or_else(|| {
                Err(format!(
                    "No mock for get_permissioned_candidates({genesis_utxo:?})"
                ))
            })
            .map_err(|err| anyhow!("{err}"))
    }
...
impl Drop for MockIOContext {
    fn drop(&mut self) {
        ...
        if let Some(next_expected) = self.expected_io.lock().unwrap().first() {
            panic!(
                "IO operations left unperformed. Next expected: {:?}",
                next_expected
            );
        }
    }
...
impl IOContext for MockIOContext {
    ...
    fn prompt_yes_no(&self, prompt: &str, default: bool) -> bool {
        let next = self.pop_next_action(&format!(
            "prompt_yes_no(prompt = {prompt}, default = {default})"
        ));
        next.print_mock_location_on_panic(|next| match next {
            MockIO::PromptYN {
                prompt: expected_prompt,
    ...
    fn offchain_impl(
        ...
        let mock = self.offchain_mocks.mocks.get(&addr.into()).ok_or_else(|| {
            anyhow::anyhow!(
                "No mock for Offchain implementation for {:?}",
                ogmios_config
            )
        })?;
        Ok(mock.clone())
