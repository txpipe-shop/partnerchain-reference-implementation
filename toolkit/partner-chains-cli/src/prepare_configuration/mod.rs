...
use partner_chains_cardano_offchain::scripts_data::{get_scripts_data, ScriptsData};
...
fn read_bootnode_defaults(context: &impl IOContext) -> (Protocol, String, u16) {
    ...
    .unwrap_or((
        Protocol::Dns,
        Protocol::Dns.default_address().to_string(),
        DEFAULT_PORT,
    ));
...
fn choose_protocol(context: &impl IOContext, default_protocol: Protocol) -> Protocol {
    ...
    protocols.sort_by_key(|protocol| {
        if *protocol != String::from(default_protocol) {
            1
        } else {
            0
        }
    ...
    Ok(match context.read_file(&network_key_path).as_deref() {
        ...
        None => {
            context.print(&format!(
                "Could not read network key from {}",
                network_key_path
            ));
            None
        }
...
pub mod tests {
    ...
    use crate::tests::{MockIO, MockIOContext, CHAIN_CONFIG_FILE_PATH, RESOURCES_CONFIG_FILE_PATH};
...
* establish Partner Chains Smart Contracts addresses and policies (to be later included in chain-spec file)",
    ...
    fn peer_id_correctly_generated_from_secret_network_key() {
        ...
        assert_eq!(
            peer_id,
            "12D3KooWSi9ys81fpG9ibuVWh6w6egfcTUM8L1iSJSpfFtMLMLG9"
        );
    }
    ...
    fn propose_saved_defaults_but_pick_different() {
        ...
        verify_json!(
            mock_context,
            CHAIN_CONFIG_FILE_PATH,
            scenarios::dns_bootnode(KEY, 3034)
        );
    ...
    fn propose_saved_defaults_and_pick_it() {
        ...
        verify_json!(
            mock_context,
            CHAIN_CONFIG_FILE_PATH,
            scenarios::ip_bootnode(KEY, 3034)
        );
    ...
    fn dont_accept_invalid_ip_address() {
        ...
        let result = PrepareConfigurationCmd {
    ...
    pub fn prompt_with_default<T>(
        ...
        MockIO::prompt(
            &format!("Enter the {}", field_definition.name),
            default,
            value,
        )
    }
