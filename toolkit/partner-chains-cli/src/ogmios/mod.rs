use crate::{config::ServiceConfig, IOContext};
use anyhow::anyhow;
use ogmios_client::{
...
pub fn ogmios_request(
    ...
    tokio_runtime.block_on(async {
        let client = client_for_url(&config.url(), Duration::from_secs(config.timeout_seconds))
            ...
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to connect to Ogmios at {} with: {}",
                    config.url(),
                    e
                )
            })?;
        ...
        vec![
            EraSummary {
                ...
                parameters: EpochParameters {
                    epoch_length: 21600,
                    slot_length_millis: 20000,
                },
            },
            EraSummary {
                start: EpochBoundary {
                    time_seconds: 1728000,
                    slot: 86400,
                    epoch: 4,
                },
                parameters: EpochParameters {
