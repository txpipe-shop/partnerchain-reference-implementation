...
use parity_scale_codec::{
    decode_vec_with_len, Compact, Decode, DecodeWithMemTracking, Encode, MaxEncodedLen,
    WrapperTypeEncode,
};
...
use sp_core::{
    ...
    ecdsa, ed25519, sr25519, ConstU32,
};
#[cfg(feature = "serde")]
use {
    derive_more::FromStr,
...
pub fn offset_data_epoch(epoch: &McEpochNumber) -> Result<McEpochNumber, u32> {
    Ok(McEpochNumber(
        epoch
            .0
            .checked_sub(DATA_MC_EPOCH_OFFSET)
            .ok_or(DATA_MC_EPOCH_OFFSET)?,
    ))
}
...
impl FromStr for AssetId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.split_once(".") {
            ...
            None => Err(
                "AssetId should be <hex encoded Policy ID>.<hex encoded Asset Name>".to_string(),
            ),
        }
    }
}
...
impl Decode for MainchainSignature {
    fn decode<I: parity_scale_codec::Input>(
        ...
        let arr = vec
            .try_into()
            .map_err(|_| "Incorrect MainchainSignature size")?;
...
impl EpochNonce {
    ...
    pub fn as_array(&self) -> [u8; EPOCH_NONCE_LEN] {
        ...
        epoch_nonce
            .try_into()
            .expect("Should never fail after being resized")
    }
}
...
impl From<Vec<([u8; 4], Vec<u8>)>> for CandidateKeys {
    fn from(value: Vec<([u8; 4], Vec<u8>)>) -> Self {
        Self(
            value
                .into_iter()
                .map(|(id, bytes)| CandidateKey { id, bytes })
                .collect(),
        )
    }
}
...
impl Encode for CandidateKeys {
    fn size_hint(&self) -> usize {
        if self.has_only_aura_and_grandpa_keys() {
            Encode::size_hint(&AuraPublicKey(self.find_or_empty(AURA))).saturating_add(
                Encode::size_hint(&GrandpaPublicKey(self.find_or_empty(GRANDPA))),
            )
        } else {
    ...
    fn from_str(partner_chain_public_keys: &str) -> Result<Self, Self::Err> {
        ...
        > {
                ...
                line.split(":").collect::<Vec<_>>()[..]
            {
                ...
                Err(
                    format!("Failed to parse partner chain public keys (legacy) from '{line}'")
                        .into(),
                )
            }
        }
