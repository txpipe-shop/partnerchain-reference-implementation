...
#[cfg_attr(
    feature = "std",
    derive(Decode, thiserror::Error, sp_core::RuntimeDebug)
)]
...
impl MainchainEpochDerivation for MainchainEpochConfig {
    ...
    fn timestamp_to_mainchain_epoch(
        ...
        Ok(McEpochNumber(
            self.first_epoch_number.saturating_add(epochs_passed),
        ))
    }
    ...
    fn first_slot_of_epoch(
        ...
        Ok(McSlotNumber(
            slots_since_first_epoch + self.first_slot_number,
        ))
    }
    ...
    fn epoch_for_slot(&self, slot: McSlotNumber) -> Result<McEpochNumber, EpochDerivationError> {
        ...
        let epochs_since_first_epoch =
            u32::try_from(slots_since_first_slot / self.slots_per_epoch())
                .map_err(|_| EpochDerivationError::EpochTooBig)?;
        Ok(McEpochNumber(
            self.first_epoch_number + epochs_since_first_epoch,
        ))
    }
}
...
mod tests {
    ...
    fn epoch_for_slot_on_preprod() {
        ...
        assert_eq!(
            config
                .epoch_for_slot(McSlotNumber(48902399))
                .expect("Should succeed"),
            McEpochNumber(116)
        );
        assert_eq!(
            config
                .epoch_for_slot(McSlotNumber(48902400))
                .expect("Should succeed"),
            McEpochNumber(117)
        );
        assert_eq!(
            config
                .epoch_for_slot(McSlotNumber(48912400))
                .expect("Should succeed"),
            McEpochNumber(117)
        );
    }
    ...
    fn epoch_for_slot_on_preprod_slot_too_small() {
        ...
        assert_eq!(
            config.epoch_for_slot(McSlotNumber(0)),
            Err(EpochDerivationError::SlotTooSmall)
        )
    }
}
