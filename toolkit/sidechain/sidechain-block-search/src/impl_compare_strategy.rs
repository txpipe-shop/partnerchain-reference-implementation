    ...
    BlockInfo: SidechainInfo<Block>,
{
    ...
    fn compare_block(
        ...
        let ordering = match epoch_block.cmp(&self.epoch) {
            Ordering::Equal => {
                match block_info
                    .get_epoch_of_block(block + 1u32.into())?
                    .cmp(&self.epoch)
                {
    ...
    BlockInfo: SidechainInfo<Block>,
{
    ...
    fn compare_block(
        ...
        let current_block_ordering = AnyBlockInSlotRange {
            slot_range: self.slot_range.clone(),
        }
        .compare_block(block, block_info)?;
        let ordering = match current_block_ordering {
            Ordering::Equal => {
                if block < self.latest_block {
                    let next_block_ordering = AnyBlockInSlotRange {
                        slot_range: self.slot_range.clone(),
                    }
