    ...
    for CommitteeMember<AuthorityId, AuthorityKeys>
{
    fn from(candidate: Candidate<AuthorityId, AuthorityKeys>) -> Self {
        match candidate {
            Candidate::Permissioned(member) => Self::Permissioned {
    ...
    TAccountId: From<ecdsa::Public>,
{
    permissioned_candidates
        ...
        .filter_map(|candidate| {
            ...
            Some(Candidate::Permissioned(PermissionedCandidate {
                account_id,
                account_keys,
            }))
        })
...
pub enum RegistrationDataError {
    ...
    #[cfg_attr(
        feature = "std",
        error("Registration with invalid mainchain signature")
    )]
    InvalidMainchainSignature,
    /// Registration with invalid sidechain signature
    #[cfg_attr(
        feature = "std",
        error("Registration with invalid sidechain signature")
    )]
    ...
    #[cfg_attr(
        feature = "std",
        error("Registration with invalid mainchain public key")
    )]
    InvalidMainchainPubKey,
    /// Registration with invalid sidechain public key
    #[cfg_attr(
        feature = "std",
        error("Registration with invalid sidechain public key")
    )]
...
pub enum PermissionedCandidateDataError {
    ...
    #[cfg_attr(
        feature = "std",
        error("Permissioned candidate with invalid sidechain public key")
    )]
    InvalidSidechainPubKey,
    /// Permissioned candidate with invalid account keys
    #[cfg_attr(
        feature = "std",
        error("Permissioned candidate with invalid account keys")
    )]
    InvalidAccountKeys,
}
...
fn verify_stake_pool_signature(
    ...
    if spo_signature.verify(
        signed_message_encoded,
        &ed25519::Public::from(stake_pool_pub_key.0),
    ) {
        Ok(())
    } else {
...
fn verify_tx_inputs(registration_data: &RegistrationData) -> Result<(), RegistrationDataError> {
    if registration_data
        .tx_inputs
        .contains(&registration_data.registration_utxo)
    {
        Ok(())
    } else {
...
mod tests {
    ...
    mod is_registration_data_valid_tests {
        ...
        fn create_parameters(
            ...
            let sidechain_signature = signing_sidechain_account
                .sign(&signed_message_encoded[..])
                .0
                .as_slice()[..64]
                .to_vec();
            ...
            (
                StakePoolPublicKey(mainchain_account.public().0),
                registration_data,
                genesis_utxo,
            )
        }
        ...
        fn should_work() {
            ...
            assert!(validate_registration_data::<AccountKeys>(
                &mainchain_pub_key,
                &registration_data,
                genesis_utxo,
            )
        ...
        fn should_not_work_if_mainchain_pub_key_is_different() {
            let (mainchain_pub_key, registration_data, genesis_utxo) = create_valid_parameters();
            let different_mainchain_pub_key = StakePoolPublicKey(
                ed25519::Pair::from_seed_slice(&[0u8; 32])
                    .unwrap()
                    .public()
                    .0,
            );
        ...
        fn should_not_work_if_sidechain_pub_key_is_different() {
            ...
            let sidechain_pub_key = ecdsa::Pair::from_seed_slice(&[123u8; 32])
                .unwrap()
                .public()
                .0
                .to_vec();
    ...
    fn should_filter_out_candidates_with_invalid_stake() {
        ...
        assert_eq!(
            valid_candidates[0].0.stake_delegation(),
            Some(StakeDelegation(1))
        );
        assert_eq!(
            valid_candidates[1].0.stake_delegation(),
            Some(StakeDelegation(2))
        );
    }
    ...
    fn should_filter_out_permissioned_candidates_with_invalid_keys() {
        let valid_sidechain_pub_key = SidechainPublicKey(
            ecdsa::Pair::from_seed_slice(&[123u8; 32])
                .unwrap()
                .public()
                .0
                .to_vec(),
        ...
        let valid_aura_key = CandidateKey {
            id: *b"sr25",
            bytes: aura_key_bytes.to_vec(),
        };
        let grandpa_key_bytes =
            hex!("88dc3417d5058ec4b4503e0c12ea1a0a89be200fe98922423d4334014fa6b0ee");
        let valid_grandpa_key = CandidateKey {
            id: *b"ed25",
            bytes: grandpa_key_bytes.to_vec(),
        };
