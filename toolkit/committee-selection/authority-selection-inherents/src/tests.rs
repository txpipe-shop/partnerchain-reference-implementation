...
use sp_runtime::{impl_opaque_keys, BoundToRuntimeAppPublic, RuntimeAppPublic};
...
fn registration_message_encoding() {
    ...
    let genesis_utxo = UtxoId {
        tx_hash: McTxHash(sample_utxo_id_hash_bytes),
        index: UtxoIndex(4),
    };
...
const ALL_MOCK_VALIDATORS: [(MockValidator, [u8; 33]); 11] = [
    (
        ALICE,
        hex!("0333022898140662dfea847e3cbfe5e989845ac6766e83472f8b0c650d85e77bae"),
    ),
    (
        BOB,
        hex!("02182879ec92e811e2a8cc117f3cde1f61d3cba0093134cfb1ed17a4ef74915d4a"),
    ),
    (
        CHARLIE,
        hex!("02f4f4d0eccb899bf2d611b56e0afec7c740efba404f8d0e82a545f988c45316c4"),
    ),
    (
        DAVE,
        hex!("03a0af06322d100056125fac1df39d161089b07ae279505aae8731c4d110a54ad7"),
    ),
    (
        EVE,
        hex!("03f045328f504c13dac9ddd9b1186098aee7c46cb8d55289dbbf2433bab7a26239"),
    ),
    (
        FERDIE,
        hex!("0325fc2095902f5fe394f244bce38b0dc3d631cbc05f0b64d5620a71bbf2514f0f"),
    ),
    (
        GREG,
        hex!("029a1eb2e31dcaf468dbb516f9b620fdd7c3f090d58a88e02b51b25255b2182dd1"),
    ),
    (
        HENRY,
        hex!("030e901c390fa37d101ff25d70594acd2df67b4493ee77a73684f25d39313536d7"),
    ),
    (
        IDA,
        hex!("03586dafcdab3d4647d4dc68732a9cab8aa34c00c5edd04e65d9dd44c2a1fd21e2"),
    ),
    (
        JAMES,
        hex!("03aec8e80ea0375f8669d6e55d7abb6a3117678d7bb851a1bd100a01e52a4fed90"),
    ),
    (
        KIM,
        hex!("03e843f200e30bc5b951c73a96d968db1c0cd05e357d910fce159fc59c40e9d6e2"),
    ),
];
...
impl MockValidator {
    ...
    pub fn keys(&self) -> CandidateKeys {
        CandidateKeys(vec![
            CandidateKey {
                id: *b"sr25",
                bytes: self.session_keys().aura.encode(),
            },
            CandidateKey {
                id: *b"ed25",
                bytes: self.session_keys().grandpa.encode(),
            },
        ])
    }
}
...
fn ariadne_3_to_2_with_more_available_candidates_test() {
    ...
    let permissioned_selected = committee_names
        ...
        .filter(|name| {
            permissioned_validators
                .iter()
                .any(|validator| validator.name == **name)
        })
        .count();
...
fn maybe_from_candidate_keys_extracts_keys_is_insensitive_to_keys_order() {
    ...
    assert_eq!(
        AccountKeys::maybe_from(&CandidateKeys(vec![key2, key1])).unwrap(),
        expected
    );
}
