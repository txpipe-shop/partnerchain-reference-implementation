...
impl AriadneInherentDataProvider {
        ...
        >,
    {
        ...
        if data_epoch < mc_reference_epoch {
            Ok(
                AriadneInherentDataProvider::from_mc_data(data_source, for_mc_epoch, scripts)
                    .await?,
            )
        } else {
...
where
    ...
    >,
{
    let next_unset_epoch = client
        .runtime_api()
        .get_next_unset_epoch_number(parent_hash)?;
