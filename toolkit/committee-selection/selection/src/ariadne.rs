...
mod tests {
    ...
    fn selects_expected_number_of_committee_member_with_expected_ratio_of_seats(
        ...
        let permissioned_candidates: Vec<_> = all_candidates
            .iter()
            .skip(trustless_num)
            .map(|(id, _)| id.clone())
            .collect();
        ...
        if num_trustless_seats > 0 && num_permissioned_seats > 0 {
            let permissioned_count = committee
                .iter()
                .filter(|id| permissioned_candidates.contains(*id))
                .count();
            let trustless_count = committee
                .iter()
                .filter(|id| trustless_candidates.contains(*id))
                .count();
