    ...
    SC: Ord + Clone,
{
    ...
    let permissioned_candidates: Vec<(SC, Weight)> = permissioned_candidates
        .into_iter()
        .map(|c| (c, 1))
        .collect();
...
pub fn weighted_with_guaranteed_assignment<T: Clone + Ord>(
    ...
    candidates.sort();
    let SelectGuaranteedResult {
        mut selected,
        remaining,
    } = select_guaranteed(candidates, n);
    let selected_count: u16 = selected
        .len()
        .try_into()
        .expect("selected count can not exceed u16");
...
mod tests {
    ...
    fn permissioned_get_p_seats_registered_get_r_seats(p: u8, r: u8, seed: TestNonce) {
        ...
        let p: u16 = p.into();
        let r: u16 = r.into();
        let registered_candidates: Vec<_> = (0..2 * r)
            .into_iter()
            .map(|i| (format!("R{i}"), 1))
            .collect();
