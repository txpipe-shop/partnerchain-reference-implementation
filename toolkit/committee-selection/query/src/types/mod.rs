...
use sidechain_domain::{byte_string::ByteString, CandidateKeys};
...
impl GetCommitteeResponse {
        ...
        Member::AuthorityId: AsRef<[u8]>,
    {
        let committee = committee
            ...
            .collect();
        GetCommitteeResponse {
            sidechain_epoch: sidechain_epoch.0,
            committee,
        }
    }
        ...
        AuthorityId: AsRef<[u8]>,
    {
        let committee = committee
            .into_iter()
            .map(|member| CommitteeMember::new(member))
            .collect();
        GetCommitteeResponse {
            sidechain_epoch: sidechain_epoch.0,
            committee,
...
impl CommitteeMember {
    ...
    pub fn new<T: AsRef<[u8]>>(bytes: T) -> Self {
        Self {
            sidechain_pub_key: to_hex(bytes.as_ref(), false),
        }
    }
}
...
mod tests {
    ...
    fn get_committee_response_to_json_test() {
        let input = GetCommitteeResponse::new(
            ...
            vec![
                (ecdsa::Public::from([0u8; 33]), 0),
                (ecdsa::Public::from([255u8; 33]), 1),
            ],
        );
