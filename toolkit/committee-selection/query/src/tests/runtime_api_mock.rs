...
use authority_selection_inherents::{
    validate_permissioned_candidate_data, validate_registration_data, MaybeFromCandidateKeys,
    PermissionedCandidateDataError, RegistrationDataError, StakeError,
};
use mock::*;
use sidechain_domain::*;
use sp_core::{Decode, Encode};
use sp_runtime::{impl_opaque_keys, BoundToRuntimeAppPublic};
    ...
    if epoch == conversion::GENESIS_EPOCH || epoch == conversion::EPOCH_OF_BLOCK_1 {
        vec![
            (
                ...
                SessionKeys {
                    key: sp_core::sr25519::Public::from([0u8; 32]).into(),
                },
            ),
            (
                CrossChainPublic([1u8; 33]),
                SessionKeys {
                    key: sp_core::sr25519::Public::from([1u8; 32]).into(),
                },
            ),
        ]
    } else {
        vec![
            (
                ...
                SessionKeys {
                    key: sp_core::sr25519::Public::from([2u8; 32]).into(),
                },
            ),
            (
                CrossChainPublic([epoch as u8 + 1; 33]),
                SessionKeys {
                    key: sp_core::sr25519::Public::from([3u8; 32]).into(),
                },
            ),
        ]
    }
