    ...
    >,
{
    ...
    fn get_current_committee_versioned(
        ...
        if self.validator_management_api_version(block)? < 2 {
            #[allow(deprecated)]
            let (epoch, authority_ids) = api
                .get_current_committee_before_version_2(block)
                .map_err(err_debug)?;
    ...
    fn get_next_committee_versioned(
        ...
        if self.validator_management_api_version(block)? < 2 {
            #[allow(deprecated)]
            Ok(api
                .get_next_committee_before_version_2(block)
                .map_err(err_debug)?
                .map(|(epoch, authority_ids)| {
    ...
    C::Api: CandidateValidationApi<Block>,
{
    fn get_epoch_committee(&self, epoch_number: u64) -> QueryResult<GetCommitteeResponse> {
        ...
        if epoch_number.is_zero() {
            ...
            return Ok(GetCommitteeResponse {
                sidechain_epoch: 0,
                ..genesis_committee
            });
        }
        ...
        if epoch_number < first_epoch {
            return Err(format!(
                "Epoch {} is earlier than the Initial Epoch!",
                epoch_number
            ));
        }

        let epoch_of_latest_block = self
            .client
            .get_epoch_of_block(latest_block)
            .map_err(err_debug)?;
        ...
        if epoch_number == epoch_of_latest_block.next() {
            self.get_next_committee_versioned(best_hash)?.ok_or(format!(
                "Committee is unknown for the next epoch: {epoch_number}"
            ))
        } else {
            let block_hash = self
                .client
                .find_block(AnyBlockInEpoch {
                    epoch: epoch_number,
                })
                .map_err(err_debug)?;
    ...
    async fn get_registrations(
        ...
        Ok(registrations_map
            .remove(&to_hex(&mc_public_key.0, false))
            .unwrap_or(vec![]))
    }
