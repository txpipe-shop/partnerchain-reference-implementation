...
pub(crate) async fn synchronize_helper(db: &Db, client: &HttpClient) -> anyhow::Result<()> {
    ...
    while Some(wallet_hash) != node_hash {
        log::debug!(
            "Divergence at height {height}. Node reports block: {node_hash:?}. Reverting wallet block: {wallet_hash:?}."
        );
...
fn color_address(address: &Address) -> colored::ColoredString {
    if address.to_string().starts_with("70") {
