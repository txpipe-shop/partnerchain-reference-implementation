...
- the initial POSIX time of a slot can be obtained as `(slot_number - zero_slot) * slot_length + zero_time`, where `zero_slot`, `zero_time` and `slot_length` are genesis configuration parameters.
- with the actual configuration, `zero_slot = 0`, `zero_time = 1747081100000` (i.e., 2025-06-12 14:51:40 UTC), `slot_length = 3000` and a deadline set in the datum of `1747081220000` (i.e., 2025-06-12 14:53:40 UTC), the `validity_interval_start` must be at least `(1747081220000 - 1747081100000) / 3000 = 40`, which means that the owner will be able to unlock the funds 40 slots after the genesis.
